...
testAvecDonnees('E2E-65 — Aucune mémorisation du dernier jour consulté', async ({ appAvecDonnees }) => {
  const entete = new SelecteursEntete(appAvecDonnees);
  const cj = new SelecteursCahierJournal(appAvecDonnees);
...
testAvecDonnees('E2E-74 — Supprimer une séance', async ({ appAvecDonnees }) => {
  ...
  await expect(appAvecDonnees.locator('body')).not.toContainText('Lecture – Compréhension de texte');
  await expect(entete.btnAnnuler).toBeEnabled();
});
...
testAvecDonnees('E2E-76 — Warning d\'absence récurrente après affichage du vendredi', async ({ appAvecDonnees }) => {
  const entete = new SelecteursEntete(appAvecDonnees);
  const cj = new SelecteursCahierJournal(appAvecDonnees);
